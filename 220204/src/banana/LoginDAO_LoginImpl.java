...
public class LoginDAO_LoginImpl implements LoginDAO {
	...
	public LoginVO findBy_IdPwd(String id, String pwd) throws Exception {
		...
		RowMapper<LoginVO> rm = new RowMapper<LoginVO>() {
            ...
            public LoginVO mapRow(ResultSet rs, int arg1) throws SQLException {
            	...
            	vo.setLogin( rs.getString("IsLogin") );
        ...
        LoginVO vo = null;
        try {
        	vo = jdbcTemplate.queryForObject("SELECT * FROM login_T WHERE Username=? AND Password=?", rm, id, pwd);
        }
	...
	public LoginVO findBy_Id(String id) throws Exception {
        ...
        LoginVO vo = null;
        try {
        	vo = jdbcTemplate.queryForObject("SELECT * FROM login_T WHERE Username=?", rm, id);
        }
        catch( EmptyResultDataAccessException e ) {
        	// 찾는 값이 없다면 null 반환
        	return null;
	...
	public String add_join(String id, String pwd) throws Exception {
		...
		if( vo != null ) {
			...
			jdbcTemplate.update("INSERT INTO login_T VALUES(?, ?,'N', 'N')", id, pwd );
			return "success";
		}
	}
}
